...
// 3. 接收并处理搜索参数 (确保匹配 homepage.php 的 name)
if (isset($_GET['from_loc']) && $_GET['from_loc'] != "") {
    $from = mysqli_real_escape_string($conn, $_GET['from_loc']);
    $where[] = "departure LIKE '%$from%'";
}

if (isset($_GET['to_loc']) && $_GET['to_loc'] != "") {
...
<div class="container">
    <div class="content-layout">
        ...
        <aside class="filter-panel">
            ...
            <p style="font-size:0.9rem;"><b>From:</b> <?= htmlspecialchars($_GET['from_loc'] ?? 'All') ?></p>
            <p style="font-size:0.9rem;"><b>To:</b> <?= htmlspecialchars($_GET['to_loc'] ?? 'All') ?></p>
            <p style="font-size:0.9rem;"><b>Date:</b> <?= htmlspecialchars($_GET['date'] ?? 'Any Date') ?></p>
            <br>
            ...
            <section class="bus-list">
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while($row = mysqli_fetch_assoc($result)): ?>
                        <div class="bus-card">
                            <div class="bus-info">
                                <h3><?= htmlspecialchars($row['bus_company']) ?></h3>
