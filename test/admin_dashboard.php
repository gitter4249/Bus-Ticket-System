...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_bus'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO buses (departure, destination, travel_date, depart_time, price) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssd", $dep, $dest, $date, $time, $price);
    ...
    <div class="card-custom">
        ...
        <?php else: ?>
            ...
            <?php else: ?>
                ...
                <table class="table table-hover align-middle">
                    <thead><tr><th>Time</th><th>Seat No.</th><th>User Email</th><th>Status</th><th class="text-end">Action</th></tr></thead>
                    <tbody>
                        <?php 
                        list($f, $t) = explode(" -> ", $sel_route);
                        $sql = "SELECT bk.id AS bid, b.depart_time, bk.seat_no, bk.user_email FROM bookings bk JOIN buses b ON bk.bus_id = b.id WHERE b.departure=? AND b.destination=? AND b.travel_date=? ORDER BY b.depart_time ASC";
                        ...
                        while($r = $res->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><strong><?=htmlspecialchars($r['user_email'])?></strong></td>
